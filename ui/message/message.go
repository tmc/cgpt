...
// RenderMessageList formats a list of messages for display
func RenderMessageList(messages []Msg, width int, maxHeight int) string {
	if len(messages) == 0 {
		return ""
	}

	var renderedMsgs []string
	divider := lipgloss.NewStyle().Foreground(lipgloss.Color("240")).Render(strings.Repeat("─", max(0, width-2))) // Ensure non-negative repeat

	renderedMsgs = append(renderedMsgs, divider)

	totalLines := 0
	linesPerMsg := make([]int, len(messages))
	for i := len(messages) - 1; i >= 0; i-- {
		msg := messages[i]
		rendered := Render(msg, width)
		lines := strings.Count(rendered, "\n") + 1
		linesPerMsg[i] = lines
	}

	messagesToShow := make([]Msg, 0)
	showEllipsis := false
	for i := len(messages) - 1; i >= 0; i-- {
		if totalLines+linesPerMsg[i] > maxHeight {
			showEllipsis = true // Need ellipsis if we stop early
			break
		}
		messagesToShow = append([]Msg{messages[i]}, messagesToShow...) // Prepend
		totalLines += linesPerMsg[i]
	}

	if showEllipsis {
		renderedMsgs = append(renderedMsgs, lipgloss.NewStyle().Foreground(lipgloss.Color("240")).Render("..."))
	}

	for _, msg := range messagesToShow {
		renderedMsgs = append(renderedMsgs, Render(msg, width))
	}

	renderedMsgs = append(renderedMsgs, divider)
	return strings.Join(renderedMsgs, "\n")
}
